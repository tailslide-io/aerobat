...
class CircuitBreaker {
  ...
  */
  ...
  // check the circuit trip

  //

  checkCircuit(ruleData) {
    const { errorThresholdPercentage, success, failure } = ruleData;
    const errorRate = this.calculateErrorPercent(success, failure);
    console.log('Error rate', errorRate);
    if (this.circuitTripped(errorRate, errorThresholdPercentage)) {
      return this.openCircuit(ruleData);
    }
    // recovery flags check
    if (!this.shouldRecover(ruleData) || !this.timeForRecovery(ruleData)) {
  ...
  shouldRecover({ circuitStatus, isRecoverable }) {
    return circuitStatus === 'recovery' && isRecoverable;
  }

  timeForRecovery({ updatedAt, circuitRecoveryRate }) {
